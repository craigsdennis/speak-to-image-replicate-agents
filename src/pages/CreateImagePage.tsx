...
export function CreateImagePage() {
  ...
  const handleSuggestPrompt = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error ?? 'Unable to generate a prompt.')
      }
      ...
      setSuggestError(err instanceof Error ? err.message : 'Failed to fetch a prompt.')
    } finally {
      ...
      <section className="flex flex-col gap-8 rounded-3xl bg-white/95 p-8 text-slate-900 shadow-2xl ring-1 ring-white/10">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-slate-500">Share your idea or ask the agent to surprise you.</p>
            <button
              type="button"
              onClick={handleSuggestPrompt}
              disabled={suggestStatus === 'loading'}
              className="inline-flex items-center justify-center rounded-full border border-indigo-200 px-4 py-2 text-xs font-semibold text-indigo-600 transition hover:border-indigo-300 hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {suggestStatus === 'loading' ? 'Generating…' : 'Generate random prompt'}
            </button>
          </div>
          <textarea
            id="prompt"
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-full bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500 px-6 py-3 text-base font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
          >
